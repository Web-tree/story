...
func SimulateMsgUpdateEvent(
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		var (
			...
			allEvent   = k.GetAllEvent(ctx)
			found      = false
		)
		for _, obj := range allEvent {
		...
		if !found {
			return simtypes.NoOpMsg(types.ModuleName, msg.Type(), "event creator not found"), nil, nil
		}
